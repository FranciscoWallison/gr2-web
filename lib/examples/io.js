...
function IO(memory) {
   ...
   */
  this.register_read_consecutive = function (
    port_addr,
    device,
    r8_1,
    r8_2,
  ...
  this.register_write_consecutive = function (
    port_addr,
    device,
    w8_1,
    w8_2,
  ...
  this.mmap_register = function (
    ...
    if (!read_func32) read_func32 = this.mmap_read32_shim;

    if (!write_func32) write_func32 = this.mmap_write32_shim;
  ...
  this.mmap_register(
    ...
    function (addr) {
      // READ8 fora da RAM
      if (!window._env_stop) {
        window._env_stop = true;
        dbg_log(
          "Read from unmapped memory space, addr=" + h(addr >>> 0, 8),
          LOG_IO
        );
        console.trace(); // mostra quem chamou (1x)
      }
      return 0; // <- muda de 0xFF pra 0 pra parar scan de string/loop
    },
    function (addr, value) {
      // WRITE8 fora da RAM (pode deixar como tá)
      dbg_log(
    ...
    function (addr) {
      ...
      return 0; // <- muda de -1 pra 0
    },
    function (addr, value) {
      // WRITE32 fora da RAM (pode deixar como tá)
      dbg_log(
        "Write to unmapped memory space, addr=" +
          h(addr >>> 0, 8) +
          " value=" +
    ...
    if (entry.write8 === empty_port_write) {
      dbg_log(
        "write8 port #" +
          ...
          " <- " +
          h(data, 2) +
          get_port_description(port_addr),
        LOG_IO
      );
    }
